...
export async function teardownTestContext() {
  ...
  if (companyIds.length > 0) {
    ...
    await prisma.jobItemProgressEntry.deleteMany({ where: { companyId: { in: companyIds } } });
    ...
    await prisma.attendanceEntry.deleteMany({ where: { companyId: { in: companyIds } } });
    await prisma.workAssignmentMember.deleteMany({ where: { companyId: { in: companyIds } } });
    await prisma.workAssignment.deleteMany({ where: { companyId: { in: companyIds } } });
    await prisma.scheduleAbsence.deleteMany({ where: { companyId: { in: companyIds } } });
    await prisma.driverRunLog.deleteMany({ where: { companyId: { in: companyIds } } });
    await prisma.workSchedule.deleteMany({ where: { companyId: { in: companyIds } } });
