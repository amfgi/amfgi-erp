...
const DEFAULT_DISPATCH_NOTE_SECTIONS: DocumentSection[] = [
  {
    type: 'image',
    heightMm: 22,
    source: 'field',
  ...
  { type: 'line', thickness: 2, color: '#334155', marginTop: 0, marginBottom: 0, width: 100 },
  { type: 'spacer', height: 1 },
  {
    type: 'heading',
    text: 'DISPATCH NOTE',
  ...
  {
    type: 'text',
    content: 'Material issued from store to job site.',
    fontSize: 8,
    align: 'center',
    bold: false,
    color: '#64748b',
  ...
  { type: 'spacer', height: 2 },
  {
    type: 'field-row',
    cells: [
      { label: 'Reference', field: 'dispatch.reference', width: 40, bold: true, fontSize: 10, color: '#0f172a' },
      { label: 'Dispatch date', field: 'dispatch.date', width: 30, fontSize: 9, color: '#334155' },
      { label: 'Printed', field: 'today', width: 30, align: 'right', fontSize: 8, color: '#64748b' },
    ],
    bordered: false,
    minHeight: 10,
  },
  ...
  { type: 'spacer', height: 2 },
  { type: 'divider', thickness: 2, color: '#334155', marginTop: 0, marginBottom: 1 },
  {
    type: 'table',
    dataSource: 'items',
    columns: [
      { header: '#', field: 'slno', width: 6, align: 'center' },
      { header: 'Material', field: 'name', width: 34, align: 'left' },
      { header: 'Warehouse', field: 'description', width: 24, align: 'left' },
      { header: 'Qty', field: 'qty', width: 14, align: 'right' },
      { header: 'Unit', field: 'unit', width: 14, align: 'center' },
    ],
    fontSize: 8,
  ...
  { type: 'spacer', height: 1 },
  {
    type: 'field-row',
    cells: [
      { label: 'Total qty', field: 'dispatch.quantity', width: 50, bold: true, fontSize: 8, color: '#334155' },
      { label: 'Total value (ref.)', field: 'dispatch.totalCost', width: 50, align: 'right', fontSize: 8, color: '#64748b' },
    ],
    bordered: false,
  },
  { type: 'spacer', height: 2 },
  {
    type: 'text',
    content: 'Issued quantities are subject to site verification.',
    fontSize: 8,
    align: 'center',
    bold: false,
    color: '#475569',
...
export const DEFAULT_DISPATCH_NOTE: DocumentTemplate = (() => {
  const base: DocumentTemplate = {
    id: 'default-dispatch-note',
    name: 'Dispatch Note',
    itemType: 'dispatch-note',
    isDefault: true,
    pageMargins: { top: 10, right: 12, bottom: 10, left: 12 },
    sections: DEFAULT_DISPATCH_NOTE_SECTIONS,
    canvasMode: true,
    canvasRects: [],
  };
  return {
...
export const DEFAULT_TEMPLATES: DocumentTemplate[] = [DEFAULT_DELIVERY_NOTE, DEFAULT_DISPATCH_NOTE];
